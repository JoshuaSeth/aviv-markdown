...
final class AppCommandCatalogTests: XCTestCase {
    ...
    func testCatalogProvidesAtLeastTwentyStandardCommandsWithoutShortcutCollisions() {
        ...
        for command in AppCommandCatalog.commands where !command.keyEquivalent.isEmpty {
            ...
            if let previous = shortcutsBySignature[signature] {
                XCTFail(
                    "Shortcut collision: \(previous) and \(command.identifier) both use \(signature)"
                )
            }
    ...
    func testFindCommandsCarryNativeTextFinderTags() {
        XCTAssertEqual(
            AppCommandCatalog.command(identifier: "find")?.tag,
            NSTextFinder.Action.showFindInterface.rawValue
        )
        XCTAssertEqual(
            AppCommandCatalog.command(identifier: "findAndReplace")?.tag,
            NSTextFinder.Action.showReplaceInterface.rawValue
        )
        XCTAssertEqual(
            AppCommandCatalog.command(identifier: "findNext")?.tag,
            NSTextFinder.Action.nextMatch.rawValue
        )
        XCTAssertEqual(
            AppCommandCatalog.command(identifier: "findPrevious")?.tag,
            NSTextFinder.Action.previousMatch.rawValue
        )
        XCTAssertEqual(
            AppCommandCatalog.command(identifier: "useSelectionForFind")?.tag,
    ...
    func testMenuBuilderKeepsEveryCatalogCommandAddressable() {
        ...
        XCTAssertTrue(
            builtItems.contains {
                $0.identifier?.rawValue == "save" && $0.target === delegateTarget
            }
        )
        XCTAssertTrue(
            builtItems.contains {
                $0.identifier?.rawValue == "print" && $0.target === delegateTarget
            }
        )
        XCTAssertTrue(
            builtItems.contains { $0.identifier?.rawValue == "paste" && $0.target == nil }
        )
    }
