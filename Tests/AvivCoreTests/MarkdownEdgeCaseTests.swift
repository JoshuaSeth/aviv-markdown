...
final class MarkdownEdgeCaseTests: XCTestCase {
    ...
    func testFortyManualMarkdownEdgeCasesStayStable() {
        ...
        for edgeCase in Self.edgeCases {
            let selection = selectionRange(
                for: edgeCase.cursorNeedle,
                in: edgeCase.markdown,
                file: #filePath,
                line: #line
            )
            ...
            let labels = MarkdownAnnotationParser.tokens(
                in: edgeCase.markdown,
                selectedRanges: [selection]
            ).map(\.label)
    ...
    func testInactiveTablesRenderByHidingSourceTextForOverlay() {
        ...
        let attributed = MarkdownStyler().attributedString(
            for: markdown,
            selectedRanges: [NSRange(location: (markdown as NSString).length, length: 0)]
        )
        let pipe = (markdown as NSString).range(of: "|").location
        let alpha =
            (attributed.attribute(.foregroundColor, at: pipe, effectiveRange: nil) as? NSColor)?
    ...
    func testActiveTableRowKeepsSourceVisibleForEditing() {
        ...
        let attributed = MarkdownStyler().attributedString(
            for: markdown,
            selectedRanges: [NSRange(location: cursor, length: 0)]
        )
        let activePipe = (markdown as NSString).range(of: "| Alpha").location
        let alpha =
            (attributed.attribute(.foregroundColor, at: activePipe, effectiveRange: nil) as? NSColor)?
            .usingColorSpace(.deviceRGB)?
            .alphaComponent
    ...
    private func assertTableVisibilityContract(
        for edgeCase: EdgeCase,
        tables: [MarkdownTableBlock],
        selection: NSRange
    ) {
        let styler = MarkdownStyler()
        if edgeCase.expectedTableBlocks > 0 {
            guard let firstCell = tables.first?.rows.first(where: { !$0.isSeparator })?.cells.first
            else {
            ...
            let inactive = styler.attributedString(
                for: edgeCase.markdown,
                selectedRanges: [
                    NSRange(location: (edgeCase.markdown as NSString).length, length: 0)
                ]
            )
            XCTAssertEqual(
                alpha(inactive, at: firstCell.contentRange.location),
                0,
                accuracy: 0.001,
                edgeCase.name
            )
            ...
            if NSIntersectionRange(selectedLine, firstCell.contentRange).length > 0 {
                XCTAssertGreaterThan(
                    alpha(active, at: firstCell.contentRange.location),
                    0.9,
                    edgeCase.name
                )
            }
            ...
            if pipe != NSNotFound {
                let attributed = styler.attributedString(
                    for: edgeCase.markdown,
                    selectedRanges: [selection]
                )
    ...
    private static let edgeCases: [EdgeCase] = [
        ...
        EdgeCase(
            "code shields markdown",
            "Use `**not bold**` here.",
            cursorNeedle: "not bold",
            expectedAnnotations: ["`", "`"]
        ),
        EdgeCase(
            "basic link",
        ...
        EdgeCase(
            "long link",
            "Read [long label](https://example.com/with/a/long/path?x=1).",
            cursorNeedle: "long label",
            expectedAnnotations: ["[long label](https://example.com/with/a/long/path?x=1)"]
        ),
        EdgeCase(
            "image syntax",
            "![Alt text](image.png)",
            cursorNeedle: "Alt text",
            expectedAnnotations: ["![Alt text](image.png)"]
        ),
        EdgeCase(
            "mixed inline",
            "Mix **bold**, _italic_, `code`, and [link](https://example.com).",
            cursorNeedle: "link",
            expectedAnnotations: ["[link](https://example.com)"]
        ),
        EdgeCase(
            "two links",
            "[one](https://one.example) and [two](https://two.example)",
            cursorNeedle: "two",
            expectedAnnotations: ["[two](https://two.example)"]
        ),
        EdgeCase(
            "checked task",
            "- [x] Done with **bold**",
        ...
        EdgeCase(
            ...
            "```swift\nlet x = 1\n```",
            cursorNeedle: "swift",
            expectedAnnotations: ["```swift"]
        ),
        EdgeCase(
            "basic table",
            "| A | B |\n| --- | --- |\n| 1 | 2 |",
            cursorNeedle: "1",
            expectedTableBlocks: 1
        ),
        EdgeCase(
            "aligned table",
            "| Left | Right |\n| :--- | ---: |\n| a | b |",
        ...
        EdgeCase(
            ...
            cursorNeedle: "site",
            expectedAnnotations: ["[site](https://example.com)"],
            expectedTableBlocks: 1
        ),
        EdgeCase(
            "table escaped pipe",
            "| Pattern | Meaning |\n| --- | --- |\n| a\\|b | escaped |",
            cursorNeedle: "escaped",
            expectedTableBlocks: 1
        ),
        EdgeCase("false table prose", "A sentence with A | B in it.", cursorNeedle: "sentence"),
        EdgeCase(
            "false table separator missing",
            "| A | B |\n| no | separator |",
            cursorNeedle: "no"
        ),
        EdgeCase(
            "fenced table false positive",
            "```\n| A | B |\n| --- | --- |\n```",
            cursorNeedle: "A"
        ),
        EdgeCase(
            "url with parens",
            "See [call](https://example.com/a_(b)).",
            cursorNeedle: "call",
            expectedAnnotations: ["[call](https://example.com/a_(b))"]
        ),
        EdgeCase("underscore in word", "snake_case should stay plain.", cursorNeedle: "snake_case"),
        EdgeCase(
            "empty line around bold",
            "\n**bold**\n",
            cursorNeedle: "bold",
            expectedAnnotations: ["**", "**"]
        ),
        ...
        EdgeCase(
            "final mixed stress",
            "### Mix [a](https://a.example) **b** _c_ `d` ~~e~~",
            cursorNeedle: "Mix",
            expectedAnnotations: ["###"]
        ),
    ]
