...
public enum MarkdownMinimapVerifier {
    public static func verify() -> MarkdownMinimapVerificationResult {
        ...
        for fixture in fixtures {
            ...
            if fixture.expectNoScroll, initialMetrics.scrollableDocumentHeight > 1 {
                failures.append(
                    "\(fixture.name): short document has phantom scroll height \(initialMetrics.scrollableDocumentHeight)"
                )
            }
            ...
            for ratio in fixture.scrollRatios {
                ...
                measuredPositions += 1
                failures.append(
                    contentsOf: verifyCurrentPosition(
                        workspace,
            ...
            workspace.minimapForTesting.scrollForTesting(
                centeredAtMinimapY: workspace.minimapForTesting.bounds.midY
            )
            settle(workspace)
            measuredPositions += 1
            ...
            if let metrics = workspace.minimapForTesting.viewportMetrics(),
                metrics.scrollableDocumentHeight > 1
            {
                workspace.minimapForTesting.scrollForTesting(
                    thumbMinY: metrics.trackRect.maxY - metrics.thumbRect.height
                )
                settle(workspace)
                measuredPositions += 1
                failures.append(
                    contentsOf: verifyBottomScroll(workspace, fixtureName: fixture.name)
                )
            }
        }
    ...
    private static func verifyCurrentPosition(
        ...
        let expectedVisibleMinY = min(
            max(0, visibleRect.minY),
            max(0, metrics.documentHeight - metrics.visibleHeight)
        )
        if abs(metrics.visibleMinY - expectedVisibleMinY) > 1 {
            failures.append(
                "\(fixtureName)@\(ratio): metrics visibleMinY \(metrics.visibleMinY) != clip visibleMinY \(expectedVisibleMinY)"
            )
        }
        ...
        if abs(metrics.thumbRect.height - exactProjectedHeight) <= 0.75 {
            if abs(metrics.thumbRect.minY - projectedTop) > 0.75 {
                failures.append(
                    "\(fixtureName)@\(ratio): thumb top \(metrics.thumbRect.minY) != projected visible top \(projectedTop)"
                )
            }
            if abs(metrics.thumbRect.maxY - projectedBottom) > 0.75 {
                failures.append(
                    "\(fixtureName)@\(ratio): thumb bottom \(metrics.thumbRect.maxY) != projected visible bottom \(projectedBottom)"
                )
            }
        } else {
            let projectedCenter = metrics.projectedY(
                forDocumentY: metrics.visibleMinY + metrics.visibleHeight / 2
            )
            ...
            if abs(metrics.thumbRect.midY - clampedCenter) > 0.75 {
                failures.append(
                    "\(fixtureName)@\(ratio): minimum thumb center \(metrics.thumbRect.midY) != projected center \(clampedCenter)"
                )
            }
        }
        ...
        if ratio == 1, metrics.scrollableDocumentHeight > 1,
            abs(metrics.thumbRect.maxY - metrics.trackRect.maxY) > 0.75
        {
    ...
    private static func verifyCenteredScroll(
        _ workspace: EditorWorkspaceView,
        fixtureName: String
    ) -> [String] {
        guard let metrics = workspace.minimapForTesting.viewportMetrics(),
            metrics.scrollableDocumentHeight > 1
        else {
            return []
        }

        let visibleCenterRatio =
            (metrics.visibleMinY + metrics.visibleHeight / 2) / metrics.documentHeight
        if abs(visibleCenterRatio - 0.5) > 0.015 {
            return [
                "\(fixtureName): minimap center click scrolled to center ratio \(visibleCenterRatio), expected 0.5"
            ]
        }
        return []
    }

    private static func verifyBottomScroll(
        _ workspace: EditorWorkspaceView,
        fixtureName: String
    ) -> [String] {
        guard let metrics = workspace.minimapForTesting.viewportMetrics() else {
        ...
        if abs(metrics.visibleMaxY - metrics.documentHeight) > 1.5 {
            return [
                "\(fixtureName): thumb drag to bottom ended at visibleMaxY \(metrics.visibleMaxY), documentHeight \(metrics.documentHeight)"
            ]
        }
        return []
    }
    ...
    private static var wrappedLongFixture: String {
        let sentence =
            "A deliberately long visual line keeps wrapping through the text container so the minimap has to follow rendered glyph fragments, not just source newline counts."
