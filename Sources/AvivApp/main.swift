...
// pitchai-allow-assume-isolated: process startup is the audited synchronous AppKit main-thread boundary.
MainActor.assumeIsolated {
        ...
        arguments.indices.contains(snapshotIndex + 1)
    {
        ...
        if let cursorIndex = arguments.firstIndex(of: "--cursor"),
            arguments.indices.contains(cursorIndex + 1)
        {
        ...
        if let zoomIndex = arguments.firstIndex(of: "--zoom"),
        ...
        if let scrollIndex = arguments.firstIndex(of: "--scroll"),
            arguments.indices.contains(scrollIndex + 1)
        {
        ...
        let markdown: String
        let baseURL: URL?
        if let markdownIndex = arguments.firstIndex(of: "--markdown"),
            arguments.indices.contains(markdownIndex + 1)
        {
            let markdownURL = URL(fileURLWithPath: arguments[markdownIndex + 1])
            markdown =
                (try? String(contentsOf: markdownURL, encoding: .utf8)) ?? MarkdownSamples.starter
        ...
        do {
            try MarkdownSnapshotRenderer.renderSample(
                to: URL(fileURLWithPath: path),
                cursorNeedle: cursorNeedle,
                viewScale: viewScale,
                markdown: markdown,
                baseURL: baseURL,
    ...
    if let printSnapshotIndex = arguments.firstIndex(of: "--snapshot-print"),
        arguments.indices.contains(printSnapshotIndex + 1)
    {
        ...
        do {
            try MarkdownSnapshotRenderer.renderPrintSample(
                to: URL(fileURLWithPath: path),
    ...
    if let minimapSnapshotIndex = arguments.firstIndex(of: "--snapshot-minimap"),
        arguments.indices.contains(minimapSnapshotIndex + 1)
    {
        ...
        do {
            try MarkdownSnapshotRenderer.renderMinimapFixture(
                to: URL(fileURLWithPath: path),
                scrollRatio: scrollRatio,
                viewScale: viewScale
            )
    ...
    if let scrollStabilitySnapshotIndex = arguments.firstIndex(of: "--snapshot-scroll-stability"),
        arguments.indices.contains(scrollStabilitySnapshotIndex + 1)
    {
